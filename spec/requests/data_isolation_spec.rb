...
RSpec.describe "Data Isolation", type: :request do
  ...
  let!(:enrollment_a) { create(:enrollment, :active, booker: booker_a, name: "Freelancer for A") }
  let!(:enrollment_b) { create(:enrollment, :active, booker: booker_b, name: "Freelancer for B") }

  let!(:booking_a) { create(:booking, enrollment: enrollment_a, description: "Work for A") }
  let!(:booking_b) { create(:booking, enrollment: enrollment_b, description: "Work for B") }
