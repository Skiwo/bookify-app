# Represents the booker-to-freelancer relationship, mirroring POP's enrollment concept.
#
# pop_worker_id      — The partner_worker_id assigned by the booker. Used to look up
#                      profiles and create payouts on POP.
# pop_enrollment_id  — POP's internal enrollment UUID. Confusingly, POP uses different
#                      field names for this across endpoints:
#                        GET /profiles/:worker_id  → returned as "enrollment_id"
#                        GET /enrollments/:id       → returned as "id"
#                      Both refer to the same value. We store it here to enable
#                      DELETE /enrollments/:id when removing a freelancer.
class Enrollment < ApplicationRecord
  ...
  def generate_invitation_token
    loop do
      ...
      break unless Enrollment.exists?(invitation_token: invitation_token)
    end
  end
