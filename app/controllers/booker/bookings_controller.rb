module Booker
  class BookingsController < BaseController
    ...
    def index
      @bookings = Booking.joins(:engagement)
        .where(engagements: { booker_id: current_user.id })
        .includes(engagement: :freelancer)
    ...
    def new
      @booking = Booking.new
      @engagements = current_user.engagements_as_booker.active
      @rate_nok = nil
      load_occupation_codes
    end

    def create
      engagement = current_user.engagements_as_booker.active.find(params[:booking][:engagement_id])
      @booking = engagement.bookings.build(booking_params)
    ...
    def pay
      ...
      if result.success?
        ...
        redirect_to booker_booking_path(@booking), alert: "Payout failed. Please try again or contact support."
      end
    end
